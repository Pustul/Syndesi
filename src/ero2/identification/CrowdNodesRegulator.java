...
public class CrowdNodesRegulator {
	...
	public void regulateTemp(CrowdData newTempData){
		long lastChangeTs;
		if((newTempData.getValue()+nodesManager.getRoomAverageValue(newTempData))/2 < CrowdController.getInstance().getRoomAveragePref(newTempData)){
	...
	public void regulateLight(CrowdData newLightData){
		long lastChangeTs;
		if((newLightData.getValue()+nodesManager.getRoomAverageValue(newLightData))/2 < CrowdController.getInstance().getRoomAveragePref(newLightData)){
			for(CrowdNode node : nodesManager.getNodesList()){
